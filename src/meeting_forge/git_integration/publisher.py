...
def _build_request(
    ...
    commit_msg = build_commit_message(title=metadata.title or metadata.meeting_id, n_docs=len(files))
...
def publish_meeting(
    ...
    if not settings.git_target_repo_path:
        raise PublishError(
            "No hay repositorio destino configurado. "
            "Define GIT_TARGET_REPO_PATH en .env"
        )
    ...
    try:
        ...
        repo_module.checkout_branch(
            target_repo, request.branch_name, base=settings.git_base_branch
        )
    ...
    try:
        ...
        logger.warning(
            "No se pudo crear el PR automáticamente: {e}. "
            "El commit y push sí se completaron.",
            e=exc,
        )
        pr_url = ""
...
def _write_publish_result(meeting_dir: Path, result: PublishResult) -> None:
    ...
    (meeting_dir / _PUBLISH_FILENAME).write_text(
